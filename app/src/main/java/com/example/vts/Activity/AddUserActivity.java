...
import com.example.vts.UserRole.UserRoleApi;
...
public class AddUserActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       GridLayoutManager gridLayoutManager=new GridLayoutManager(AddUserActivity.this,2, GridLayoutManager.VERTICAL,false);
        ...
        PermissionAdapter usersAdapter1   = new PermissionAdapter(AddUserActivity.this,jsoncreate());
                            ...
                            if(data.getUserRoleId()==country.getId()){
                                try {
                                    ...
                                    for (int i = 0; i <array.length() ; i++) {
                                       ...
                                       LinearLayoutManager linearLayoutManager = new LinearLayoutManager(AddUserActivity.this);
                                        permissions.setLayoutManager(gridLayoutManager);
                                        PermissionAdapter usersAdapter1   = new PermissionAdapter(AddUserActivity.this,permissionGetsetList);
                                        permissions.setAdapter(usersAdapter1);
    ...
    @Override
    public void onClick(View v) {
        System.out.println("hjhjhjghjg"+Valuespin);
        if(Valuespin.equalsIgnoreCase("Choose Roles")){
            Toast.makeText(AddUserActivity.this, "Please Choose Role", Toast.LENGTH_SHORT).show();
        }else {
            category();
        }
    }
    ...
    private void category() {
        ...
        AddDeviceApi.getRetrofitInstance(context).create(UserRoleInterface.class).registration3(prameter).enqueue(new Callback<Usergetset>() {
            ...
            public void onResponse(Call<Usergetset> call, Response<Usergetset> response) {
                ...
                System.out.println("jkjkjkjh"+response.body());
                if(response.body().getSuccess()==false){
                    Toast.makeText(AddUserActivity.this, response.body().getErrorMessage(), Toast.LENGTH_SHORT).show();
                }else if( response.body().getSuccess()==true){
                    Intent intent=new Intent(AddUserActivity.this,UserActivity.class);
                    intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                    startActivity(intent);
                    Toast.makeText(AddUserActivity.this, response.body().getSuccessMessage(), Toast.LENGTH_SHORT).show();
                }
            }
